...
- **The cap of 5 bounds reviewers, not the work.** For a genuinely large work set (a codebase-wide migration or audit), execution can hand off to a native dynamic workflow (the `ultracode` keyword, on builds that support it) that orchestrates many more subagents under the runtime, while `/dispatch` keeps its distinct role — the routing decision, reviewer-pool sizing, and handoff logging the native feature does not provide.
