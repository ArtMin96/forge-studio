...
- **Successful `git commit` output is compressed to `ok`.** The commit SHA, file-change summary, and `create mode` lines are dropped (~97% byte reduction). Failures pass through unchanged with the original exit code. If you need the new SHA after a commit, `git rev-parse HEAD` or `git log -1 --format=%H` recovers it.
- **Shell aliases shadow passthrough commands.** Claude Code captures your interactive shell aliases into a per-session snapshot and replays them in every Bash tool call. rtk's hook *correctly* passthroughs some commands (e.g. `find … | head` — piping breaks rtk's compact output format), and when it does, the raw command is executed under your aliases. If you have `alias find=fd`, `alias grep=rg`, `alias cat=bat`, etc., the passthrough runs the replacement tool — whose flags often don't match the original (e.g. `find -not` → `fd` errors with `unexpected argument '-n'`). Guard such aliases with Claude Code's env marker:

  ```zsh
  # ~/.config/zsh/src/aliases (or wherever you define them)
  [[ -z "$CLAUDECODE" ]] && alias find="fd"
  [[ -z "$CLAUDECODE" ]] && alias grep="rg"
  ```

  Interactive terminal keeps the replacements; Claude Code sees the real binaries.
