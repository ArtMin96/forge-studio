...
when_to_use: Reach for this during architecture choices, before locking in a refactor approach, or when consensus feels too easy. Do NOT use for reviewing code that already exists — use `/challenge` for self-review, `/verify` for diff verification, or `/assess-proposal` for the SEPL gate instead; this skill argues forward-looking design decisions.
