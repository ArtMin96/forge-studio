...
| ssl-audit | `/ssl-audit` | Audits SKILL.md frontmatter for the SSL overlay fields (`scheduling`, `structural`, `logical`). Reports skills missing a measurable success criterion; highlights routing/dispatch skills (matching `rout(e|ing)\|dispatch\|orchestrate`) as higher-priority gaps per arXiv:2605.26112 §4.3 | Before promoting a plugin, or when `/entropy-scan` flags skill quality concerns |
