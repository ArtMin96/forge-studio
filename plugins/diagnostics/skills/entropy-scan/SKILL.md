...
Run all 6 checks. Report results in the structured format below. **Do not modify any files** — report issues and propose fixes only.
...
4. **Hook Exit Codes**: Grep for `exit 2` in hooks not under PreToolUse (informational — may have false positives)
