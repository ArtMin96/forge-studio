...
when_to_use: Reach for this at the start of a new session to pick up where the previous one left off, or after a `/compact` when context state has been lost. Do NOT use for writing resume artifacts — use `/progress-log` or `/feature-list` instead; session-resume is the read-side briefer.
