...
| `turn-id.sh --from-stdin` | Reads hook JSON payload on stdin and emits `<session_id>-<hook-shell-pid>.<starttime>`. Groups writes within one hook invocation; join across sibling hook events on `session_id`, not `turn_id`. | `jsonl-append.sh` (internally), `collect-*.sh` trace collectors |
