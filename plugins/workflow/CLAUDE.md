...
- **Invocation contract.** Skills call `bash plugins/workflow/skills/orchestrate/scripts/find-active-plan.sh` from repo root; hooks call it via `${CLAUDE_PLUGIN_ROOT}/workflow-orchestrate/scripts/find-active-plan.sh` (the symlinked path). Returns the absolute plan path on stdout or empty string + exit 0 when no plan exists — always check `[[ -z "$PLAN_PATH" ]]` before using.
