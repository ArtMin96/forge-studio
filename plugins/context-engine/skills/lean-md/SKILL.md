...
when_to_use: Reach for this on a working CLAUDE.md that has accumulated cruft over many sessions, or before sharing a project's CLAUDE.md as a template. Do NOT use for section-level reorganization — use `/md-structure` instead (moving sections, adding headings); lean-md cuts content, not architecture.
