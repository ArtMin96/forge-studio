...
# Restore verification tracking state
VERIFY_DIR="${CLAUDE_PLUGIN_DATA:-/tmp/claude-test-nudge}/${SESSION_ID}"
if [[ -f "${VERIFY_DIR}/edit-count" ]]; then
  EDIT_COUNT=$(cat "${VERIFY_DIR}/edit-count")
  if [[ "${EDIT_COUNT:-0}" -gt 0 ]] && [[ ! -f "${VERIFY_DIR}/verified-since-edit" ]]; then
    OUTPUT+="- Unverified edits: ${EDIT_COUNT} edits without test verification"$'\n'
  fi
fi
