...
|-------|---------|-------------|-------------|
| change-manifest | `/change-manifest` | Appends one structured entry to `.claude/evolution/change_manifest.jsonl`. Each entry captures type, description, affected files, failure pattern, predicted fixes, risk tasks, and constraint level (AHE p.20 schema). After each write, triggers automatic rotation: when the file exceeds `FORGE_LEDGER_MAX_ENTRIES` (default 2000) entries or `FORGE_LEDGER_MAX_BYTES` (default 1 MiB), the live file is moved to `.claude/evolution/archive/<basename>-<iso-date>-<epoch>.jsonl` | When a meaningful change was made and the hook could not auto-detect it; normally fires automatically via `manifest-writer.sh`. Direct invoke: `bash plugins/forge-meta/skills/change-manifest/scripts/append-manifest.sh --type <t> --description <d>` |
| evolution-history | `/evolution-history [--include-archive]` | Renders `change_manifest.jsonl` as a reverse-chronological Markdown timeline grouped by date, capped at the last 200 entries. Skips malformed lines silently. Pass `--include-archive` to include rotated archive files from `.claude/evolution/archive/` in chronological order before the live entries | After a sprint or series of generator passes when you want a human-readable audit trail of what changed and what agents touched which files; use `--include-archive` to see entries that were rotated out of the live file |
...
| auto-tune-skill | `/auto-tune-skill <plugin>:<skill-id>` | Outer-loop proposer that iterates on a SKILL.md body (body only — frontmatter is immutable). Each iteration dispatches K=`FORGE_AUTO_TUNE_K` (default 3) `context: fork` mutation subagents, scores each candidate via `score-candidate.sh` (swap-restore bench.py run), and tracks the Pareto frontier on (pass_rate, token_cost). `pareto-best.py` selects the lex-best (max pass_rate, tie-break min token_cost). Loop runs up to `FORGE_AUTO_TUNE_ITERS` (default 3) iterations or until pass_rate ≥ 1.0. Final Pareto-best written to `.claude/proposals/<plugin>-<skill>-<timestamp>.md` — original SKILL.md never modified. Env: `FORGE_AUTO_TUNE_ITERS`, `FORGE_AUTO_TUNE_K`, `FORGE_AUTO_TUNE_MOCK=1` (skip real API calls for smoke tests). Scripts: `run-iteration.sh` (workspace setup + ledger), `score-candidate.sh` (swap-restore scorer), `pareto-best.py` (Pareto frontier + lex selection) | When a skill's eval pass rate is below target or its `when_to_use` keeps misfiring and you want a data-driven rewrite of the skill body reviewed before applying |
| manifest-analyze | `/manifest-analyze [--include-archive]` | Reads `.claude/evolution/change_manifest.jsonl` (and optionally rotated archive files) and emits a 5-section Markdown report: **Volume** (total entries, date range, session count), **Failure-pattern frequency** (top-10), **Risk-task frequency** (all values ranked), **Constraint-level distribution**, and **Why-this-component clusters**. Output is deterministic | After a sprint or extended tuning run to understand which failure patterns dominate, which risk tasks recur, and whether soft vs. hard constraints are balanced; pass `--include-archive` to include entries that were rotated out of the live file |
| skill-staleness-audit | `/skill-staleness-audit [--format=human\|json] [--threshold-stale=0.5] [--threshold-aging=0.75]` | Score every SKILL.md against staleness signals (last-edit age, eval coverage, citation freshness, frontmatter compliance) and emit a ranked report. Read-only — never modifies a skill. | Before running `/auto-tune-skill` on an unfamiliar surface, or as a quarterly maintenance pass to surface stale skills. |
