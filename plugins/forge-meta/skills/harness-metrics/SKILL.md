...
logical: a 6-row Markdown table appears in the response with numeric values or "n/a" for each dimension; .claude/metrics/<YYYY-MM-DD>.json is written
---

# /harness-metrics — Harness Quality Scorecard

Computes six harness-level dimensions defined in arXiv:2605.18747 §5.2.1 and renders them as a Markdown table. Each dimension is derived from artifacts already on disk — no inference, no fabrication.
...
Input: manifest with 4 entries, all having absent/null/empty `evidence_bundle`.
